...
class MyProblem2D(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs): # Mutivar fitness function
        ...
        out["F"] = [-m3, k] # k number of connected components (3D), 2D [-m3, k]
...
def worker2D(name):
    ...
    elementwise_problem = MyProblem2D(g1,g2,g3,a,vertices,xl,d,binary_links,n_var,lower)
    ...
    df_2d_legacydata_legacycode = pd.DataFrame(columns=cols, data=temp_results2)
    print("Before duplicate removal, size :", df_2d_legacydata_legacycode.shape)
    df_2d_legacydata_legacycode.drop_duplicates(keep="first",inplace=True)
    print("After duplicate removal, size :", df_2d_legacydata_legacycode.shape)
    temp_results2 = df_2d_legacydata_legacycode.values.tolist() #TODO is tolist necessary?
